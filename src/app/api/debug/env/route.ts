...
export async function GET(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && debugKey !== 'neon-debug-2024') {
    ...
    const dbEnvVars = {
      ...
      POSTGRES_PRISMA_URL: process.env.POSTGRES_PRISMA_URL ? 'SET (hidden)' : 'NOT_SET',
      POSTGRES_URL_NON_POOLING: process.env.POSTGRES_URL_NON_POOLING ? 'SET (hidden)' : 'NOT_SET',
      POSTGRES_USER: process.env.POSTGRES_USER ? 'SET' : 'NOT_SET',
      POSTGRES_HOST: process.env.POSTGRES_HOST ? 'SET' : 'NOT_SET',
      POSTGRES_PASSWORD: process.env.POSTGRES_PASSWORD ? 'SET (hidden)' : 'NOT_SET',
      POSTGRES_DATABASE: process.env.POSTGRES_DATABASE ? process.env.POSTGRES_DATABASE : 'NOT_SET',
      JWT_SECRET: process.env.JWT_SECRET ? 'SET (hidden)' : 'NOT_SET',
    };

    // Check which URL would be used by Prisma
    const selectedUrl = process.env.POSTGRES_PRISMA_URL || 
