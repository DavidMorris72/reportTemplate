...
function createPrismaClient() {
  ...
  if (!databaseUrl) {
    if (process.env.NODE_ENV === "production") {
      ...
      console.warn(
        "No database URL available, skipping Prisma client creation",
      );
    }
    return null;
  }
  ...
  try {
    ...
    console.log(
      `Prisma client created with database URL: ${databaseUrl.substring(0, 20)}...`,
    );
    return client;
  } catch (error) {
...
// Cache the client globally in all environments to avoid recreation
